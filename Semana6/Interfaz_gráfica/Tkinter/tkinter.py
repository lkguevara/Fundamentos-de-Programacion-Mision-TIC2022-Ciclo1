"""Paso 1. Instalación de la librería Thinker"""
...
ventana_principal.title("Mi Primera Interfaz") #Titulo de la interfaz
#ventana_principal.mainloop() #Ejecución

"""Paso 3. Frames / Marco"""

main_frame = Frame(ventana_principal,         #Ventana que va a contener el frame
                   bg="#F7DBF0",        #background color código html - > RGB Red Green Blue
                   height = 500,        #tamaño de alto
                   width = 500,         #tamaño ancho
                   padx = 600,           #margen en el eje x
                   pady = 330,           #margen en el eje y
...
title_label = Label(main_frame,            #Frame que contendrá el label
                    text = "Hola Mundo",    #Text que se va a visualizar
                    font = ('Arial',20),    #tipo de ltra y tamaño
                    fg = '#000000',         #color de la letra
                    justify = CENTER,       #Alineación LEFT, RIGHT, CENTER
                    bg = '#F7DBF0')         #Fondo del texto
title_label.pack()
# ventana_principal.mainloop()
...
def accion_button():
    global boton_text
    if boton_text == "Bienvenidos":
        boton_text = "Adiós"
    else:
        boton_text = "Bienvenidos"
    boton.config(text = boton_text)

boton_text = "Bienvenidos"
boton = Button(main_frame,
               text = boton_text,   #text coloca el texto al boton
               bg = "#F7DAD9",  #color de fondo
               fg = "#0A1931",  #color de la letra
...
def saludo():
    print("Hola, estoy aprendiendo ux")
